...
const Navbar = () => {
  ...
  const cart = [1];
  return (
    <div className="bg-red-500 text-white fixed top-0 left-0 right-0 px-5 md:px-0">
        ...
        <nav className="flex items-center gap-5 text-gray-300">
          ...
          <Link to="/cart">
            <div className="relative">
              {user && cart.length > 0 && (
                <span className="size-4 bg-primary text-white rounded-full -top-2 -left-2 absolute text-sm flex items-center justify-center">
                  {cart.length}
                </span>
              )}
              <ShoppingCart size={22} className="hover:text-white" />
            </div>
          </Link>
          ...
          {isAdmin && (
            <Link
              to="/dashboard"
              className="bg-accent text-white px-2 py-1 rounded-lg text-sm font-bold"
            >
              Admin
            </Link>
          )}
          ...
          {!user && (
            <div className="flex items-center gap-3">
              ...
              <button className="bg-surface text-gray-300 flex items-center gap-1 px-2 py-1 font-semibold rounded-lg cursor-pointer text-sm">
